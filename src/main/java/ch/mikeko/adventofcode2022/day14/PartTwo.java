...
public class PartTwo {
    public static void main(String[] args) {
        try (Stream<String> lines = InputParser.getInputByLine(14, InputType.PUZZLE_INPUT)) {
            ...
            System.out.printf("Part two result: %s%n", count);
        }
    }
    ...
    private static boolean addSand(Set<Point> points, Point bottomLeftPoint, Point bottomRightPoint) {
        var floorY = Math.max(bottomLeftPoint.getY(), bottomRightPoint.getY()) + 2;
        var sand = new Point(500, 0);
        sand.setSymbol("o");
        var canMove = true;
        while (canMove) {
            // -1. Is there already sand at 500/0? If so, the cave is full.
            if (points.contains(sand)) {
                return false;
            }

            // 0. Is the next spot the floor? If so, we can't go anywhere anymore.
            if (sand.getY() + 1 == floorY) {
                canMove = false;
                continue;
            }
